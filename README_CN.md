...
**无需任何服务端配置**——每个用户在网页上绑定**自己的**微信账号即可：

1. 打开网页端「账号 → 聊天工具绑定」
2. 滚动到**微信 ClawBot** 区块，点击**扫码登录微信**
3. 用手机微信扫二维码（登录流程要求时输入数字配对码）
4. 手机微信里出现一个 **ClawBot** 会话——直接和它聊天即可派任务

微信 ClawBot 支持：

| 能力 | 说明 |
|---|---|
| 派任务 | 发普通文本 = 给选中工作区下指令（没选过先 `/swarm select`） |
| 详细流 | 微信自己派的任务实时推 💭 思考 + 🔧 工具行（✓/✗）+ 最终回答全文 |
| 监控同步 | 开启 `monitor on` 后，你在 opencode TUI 的对话实时推送到这个 ClawBot 会话 |
| 完成简报 | 其它来源（网页/agent/A2A 外部）的任务完成后推简报（默认开，`brief off` 关） |
| 权限/提问 | 渲染为编号文本卡：回复 **1. 允许一次 / 2. 始终允许 / 3. 拒绝**（纯数字） |
| 命令菜单 | `/q` 或 `/swarm` 返回编号菜单，`/1`–`/7` 执行对应项 |

聊天命令：`/q`（菜单）· `/swarm select|list|status|last` · `/swarm monitor on|off` · `/swarm brief on|off` · `/time` · `/重新连接`。微信**没有** `/swarm bind`——绑定一律走网页扫码（一人一号，无需 API Key）。

> 注意：仅私聊（不支持群）；有待应答的权限/提问时**一切输入**优先作为应答（纯数字应答，超出 1–3 或非数字 = 允许一次）；登录 token 约 24h 失效需重新扫码。跨渠道：TUI / A2A 任务触发权限/提问且简报开着时，web / 飞书 / 微信**同时**收到卡，谁先应答谁生效。
...
> 微信（ClawBot）**无服务端环境变量配置**——按用户激活：在网页账号页扫码绑定的**自己**的微信（见上文「接入微信」）。
