...
- REST mirror for call records: `/api/calls` (list) + `DELETE /api/calls/{id}` (only terminal states, ownership checked). Timeout: `AGENT_SWARM_CALL_TIMEOUT` (default 1h), enforced in `_wait_final`/SSE generator/a2a_task lazily.
